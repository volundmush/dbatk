...
namespace dbat {
    ...
    public:
        ...
        [[nodiscard]] virtual double getDefault(entt::entity ent) const {return 0.0;};
        // this function will be called by iterating through all stats when a new thing is created.
        void setDefault(entt::entity ent) {setBase(ent, getDefault(ent));};
        [[nodiscard]] virtual std::optional<double> getMinimumBase(entt::entity ent) const {return std::nullopt;};
        [[nodiscard]] virtual std::optional<double> getMaximumBase(entt::entity ent) const {return std::nullopt;};
        ...
        [[nodiscard]] double setBase(entt::entity ent, double value) {
            ...
            if(auto max = getMaximumBase(ent)) {
        ...
        [[nodiscard]] virtual double clampEffective(entt::entity ent, std::unordered_map<std::size_t, double>& cache) {
            ...
            if(auto min = getMinimum(ent)) {
                if(value < *min) value = *min;
            }

            if(auto max = getMaximum(ent)) {
                if(value > *max) value = *max;
            }
            return value;
        }
