...
namespace dbat {
    ...
    public:
        ...
        void handleConnectCommand(const std::string& text);
        void handleBadMatch(const std::string& text, std::unordered_map<std::string, std::string>& matches);
        OpResult<int64_t> createAccount(std::string_view userName, std::string_view password);
        void onCreateAccount(std::string_view userName, std::string_view password, int64_t acc);
        OpResult<> handleLogin(const std::string &userName, const std::string &password);
        void loginToAccount(int64_t acc);
        void onLogin();
        void handleLoginCommand(const std::string& text);
        void createOrJoinSession(entt::entity ent);
